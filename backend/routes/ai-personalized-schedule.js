// AI Personalized schedule generation
// AI learns preferences, predicts optimal time to push content to viewer
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const response = await fetch(baseUrl + '/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIRecommendationsforbroadcasters'
    },
    body: JSON.stringify({
      model: MODEL,
      messages: [
...
router.post('/', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert AI assistant for AIRecommendationsforbroadcasters. Focus area: Personalized schedule generation. ${`AI learns preferences, predicts optimal time to push content to viewer`}. Respond ONLY with valid JSON (no markdown fences).`;
    const userPrompt = `Task: Personalized schedule generation.\n${`AI learns preferences, predicts optimal time to push content to viewer`}\n\nInput payload (JSON):\n${JSON.stringify(context, null, 2)}\n\nReturn JSON with the shape:\n{\n  "summary": "...",\n  "findings": ["..."],\n  "recommendations": ["..."],\n  "score": 0,\n  "confidence": 0\n}`;
    ...
    await persistResult(req.user?.id, 'personalized-schedule', context, parsed);
    res.json({ feature: 'personalized-schedule', model: MODEL, result: parsed });
  } catch (err) {
    console.error('[personalized-schedule]', err.message);
